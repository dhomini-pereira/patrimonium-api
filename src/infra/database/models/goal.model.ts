...
export const GoalModel = p.pgTable('goals', {
  id: p.uuid().primaryKey().defaultRandom(),
  userId: p.uuid().notNull().references(() => UserModel.id, { onDelete: 'cascade' }),
  name: p.varchar({ length: 255 }).notNull(),
  targetAmount: p.decimal('target_amount', { mode: 'string', precision: 2, scale: 15 }).notNull(),
  currentAmount: p.decimal('current_amount', { mode: 'string', precision: 2, scale: 15 }).default('0'),
  deadline: p.date(),
