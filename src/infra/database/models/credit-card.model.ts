...
export const CreditCardModel = p.pgTable('credit_cards', {
  id: p.uuid().primaryKey().defaultRandom(),
  userId: p.uuid('user_id').references(() => UserModel.id, { onDelete: 'cascade' }).notNull(),
  name: p.varchar({ length: 255 }).notNull(),
  cardLimit: p.decimal('card_limit', { mode: 'string', precision: 2, scale: 15 }).notNull().default('0'),
  ...
  bestPucharseDay: p.integer('best_pucharse_day').default(sql`NULL`),
}, (table) => [
  ...
  p.check('best_pucharse_day_check', sql`${table.bestPucharseDay} BETWEEN 1 AND 31`),
]);
