...
export const TransactionModel = p.pgTable('transactions', {
  id: p.uuid().primaryKey().defaultRandom(),
  userId: p.uuid('user_id').notNull().references(() => UserModel.id, { onDelete: 'cascade' }),
  accountId: p.uuid('account_id').references(() => AccountModel.id, { onDelete: 'set null' }),
  categoryId: p.uuid('category_id').references(() => CategoryModel.id, { onDelete: 'set null' }),
  ...
  recurrence: p.varchar({ length: 20, enum: ['daily', 'weekly', 'monthly', 'yearly'] }),
  ...
  creditCardId: p.uuid('credit_card_id').references(() => CreditCardModel.id, { onDelete: 'set null' }),
  installment: p.integer(),
  installmentCurrent: p.integer('installmentCurrent'),
  familyMemberId: p.uuid('family_member_id').references(() => FamilyMemberModel.id, { onDelete: 'set null' }),
}, (table) => ([
  ...
  p.index('idx_transactions_recurring').on(table.recurring, table.nextDueDate).where(sql`recurring = true`),
  p.index('idx_transactions_date_only').on(table.date),
  p.index('idx_transactions_group').on(table.recurrenceGroupId).where(sql`recurrence_group_id IS NOT NULL`),
  p.index('idx_transactions_credit_card').on(table.creditCardId).where(sql`credit_card_id IS NOT NULL`),
]));
